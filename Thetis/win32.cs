...
namespace Thetis
{
    ...
    unsafe class Win32
    {
        ...
        public static Int32 PublicWSACleanup()
        {
            int ret = WSACleanup();

            if (ret != 0)
            {
                Debug.WriteLine("Error cleaning up winsock was: " + Marshal.GetLastWin32Error()
                    + " This was expected as we are trying to second-guess .NET sockets here :-(");
                // Debug.Assert(ret == 0);
            }
            return ret;
        }
