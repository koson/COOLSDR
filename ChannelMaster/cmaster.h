...
// extern HANDLE prioritise_thread_max();
...
typedef struct _cmaster {
    ...
    int cmMAXInbound[cmMAXstream]; // maximum number of samples in a call to
                                   // Inbound()
    ...
    void (*Outbound)(int id, int nsamples,
        double* buff); // pointer to Outbound function
    ...
    struct _xmtr {
        ...
        void(__stdcall* pushvox)(
            int channel, int active); // vox, per transmitter
...
extern __declspec(dllexport) void SendpOutbound(
