...
volatile int MetisLastRecvSeq = 0;
...
PORT const char* GetWindowsErrorString(int err) {
...
int SendStartToMetis(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        MetisReadDirect((unsigned char*)&inpacket, 5000);
...
PORT int SendStopToMetis() {
    ...
    starting_seq = MetisLastRecvSeq;
    int sent = 0;
    const int sz = sizeof(outpacket);
    for (i = 0; i < 5;
         i++) { // why is this sent 5 times? Just to be sure? Dunno!
        sent = sendPacket(listenSock, (char*)&outpacket, sz, 1024);
        if (sent == sz) {
            break;
        }
        if (sent == -1) {
        ...
        if (rc < 0) {
            if (rc == -1) // SOCKET_ERROR
            {
                ...
                if (e == WSAEWOULDBLOCK) {
                    ...
                    if (since_got_data > REASONABLE_TIMEOUT) {
                        // we could notify the client here, should he care to
                        // look
                        HaveSync = 0;
                    }
                    Sleep(5);
                    ...
                    fprintf(stderr, "MetisReadDirect failure, with %s\n",
                        GetWindowsErrorString(e));
                    assert(0);
                }
            }
            assert("Dunno what this is: -1 is all we should ever get from "
                   "recvfrom()"
                == 0);
            return rc; // Dunno what this is: -1 is all we should ever get from
                       // recvfrom()
        } else if (rc > 0) {
    ...
    if (total == EXPECTED_FRAME_SIZE) { /* looks like a data frame */
        if ((inpacket.readbuf[0] == 0xef) && (inpacket.readbuf[1] == 0xfe)
            ...
            if (endpoint == 6) {
                ...
                xpro(prop, seqnum,
                    (char*)bufp); // resequence out of order packets
...
int MetisReadDirect(unsigned char* bufp, int timeoutms) {
    struct indgram {
    ...
    while (got < 1032) {
        ...
        if (rc == -1) // SOCKET_ERROR
        {
            ...
            if (dwErr == WSAEWOULDBLOCK) {
                ...
                Sleep(1);
                if (timeoutms > 0) {
                    DWORD took = timeGetTime() - startEnter;
                    if (took > (DWORD)timeoutms) {
                        return 0;
                    }
                }
                return 0;
...
#define POLL_INSTEAD
// for Protocol1 -- P2 uses ReadThreadMainLoop in network.c
