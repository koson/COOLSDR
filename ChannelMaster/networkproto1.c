    ...
    while (io_keep_running && total < EXPECTED_FRAME_SIZE) {
        ...
        if (rc < 0) {
            if (rc == -1) // SOCKET_ERROR
            {
                ...
                err = getsockopt(
                    listenSock, SOL_SOCKET, SO_ERROR, (char*)(&errNum), &len);
                if (err) {
                    show_windows_error(
                        "MetisReadDirectPOLL: getsockopt error", err);
                    assert(
                        "Unexpected error on listenSock in MetisReadDirectPOLL"
                        == 0);
                }
                if (e == WSAEWOULDBLOCK) {
                    loops++;
                    if (loops % 100 == 0) {
                        ...
                        DWORD since = now - time_last_slept;
                        if (since > 5) {
                            Sleep(1);
                            time_last_slept = timeGetTime();
                        }
