...
// This is good for WSAGetLastError as well as GetLastError
PORT const char* GetWindowsErrorString(int err);

#ifndef PRIO_THRD_DEFINED
#define PRIO_THRD_DEFINED

static inline void show_windows_error(const char* where, DWORD e) {
#ifndef NDEBUG
    assert(where != 0);
    const char* serr = GetWindowsErrorString((int)e);
    fprintf(stderr, "Where: %s, error was %ld:%s\n", where, (int)e, serr);
    fflush(stderr);
#endif
}
...
static inline HANDLE prioritise_thread_max() {
    ...
    DWORD taskIndex = 0;
    DWORD existing = GetThreadPriority(GetCurrentThread());
    // Set this first, so that at least if AvSetMMThread ... fails, we do the
    // best we can.
    BOOL ok
        = SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_TIME_CRITICAL);

    if (!ok) {
        show_windows_error("Regular SetThreadPriority in prioritise_thread_max "
                           "failed",
            GetLastError());
        assert(
            "Regular SetThreadPriority in prioritise_thread_max failed" == 0);
    }
    ...
    if (!ok) {
        ...
        if (dw != 1552) { // the specified thread is already joining a task
            show_windows_error("AvSetMmThreadPriority failed", GetLastError());
        }
    }
